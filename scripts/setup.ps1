...
<#
.SYNOPSIS
    Outfit Studio — Windows development setup.
...
Write-Header 'Outfit Studio - Windows Setup'
...
if (-not $SkipModels) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host '[WARN] Model download failed - run manually: .\.venv\Scripts\python.exe -m outfit_studio.scripts.download_models' -ForegroundColor Yellow
    }
} else {
...
Write-Host "  Run app:   .\.venv\Scripts\python.exe -m outfit_studio.main"
Write-Host "  Debug:     set OUTFIT_STUDIO_PIPELINE_DEBUG=true in .env"
Write-Host ""
